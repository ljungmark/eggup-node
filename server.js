...
const path = require('path'),
  ...
  strategies = require('./credentials/strategies'),
...
app.post('/request', (request, response) => {
  ...
  */
  ...
  let locked_from_new_orders = new Promise(function(resolve, reject) {
    let sql = 'SELECT lockdate, startdate, softboiled, hardboiled FROM cookings WHERE DATE(lockdate) = ?',
      values = [date];
    sql = mysql.format(sql, values);
...
app.post('/orders', (request, response) => {
  ...
  let check = new Promise(function(resolve, reject) {
    sql = 'SELECT tokens.token, orders.quantity, orders.variant, orders.checkout, tokens.email, tokens.name FROM orders LEFT JOIN tokens ON tokens.token = orders.token WHERE DATE(orders.date) = ? ORDER BY tokens.name ASC',
