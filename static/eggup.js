...
samples.load('click', `assets/click.mp3`);
samples.load('open', `assets/open.mp3`);
samples.load('close', `assets/close.mp3`);
samples.load('change', `assets/change.mp3`);
samples.load('error', `assets/error.mp3`);

Eggup.prototype.notify = function(sound = null) {
  const instance = this;

  if (instance.cache.notify && sound !== null && ['done', 'start', 'click', 'open', 'close', 'change', 'error'].includes(sound)) {
    samples.start(`${sound}`);
  }
